...
class RemoteClient:
    ...
    def start_server(self):
        ...
        server_config.SERVER_SOCKET = self
    ...
    #                     # It returns (Command_obj) or (error_message, error_code).
    ...
    #         return ErrorResponse("Command not provided").to_dict()), 400
    #
    ...
    #         return ErrorResponse(f"Command execution failed: {str(e)}").to_dict())
    ...
    def health_check_endpoint(self):
        ...
        return APIResponse.SystemInfoResponse(message="Health check successful", system_data={
                "name": self.name,
                "status": "healthy",
                "last_health_check": self.last_health_check
            }).to_response()

    def get_command(self, command_name: str) -> CommandEndpoint | None:
        """
        ...
        if not self.commands_loader:
            logger.error(f"{__file__} - Commands loader is not initialized.")
            return None
        if command_name not in self.commands_loader:
            logger.error(f"{__file__} - Command '{command_name}' not found in commands loader.")
            return None
        command_endpoint, code = self.commands_loader[command_name]
        if code != 200:
            logger.error(f"{__file__} - Command '{command_name}' not found.")
            return None
    ...
    def list_processes(self):
        ...
        return APIResponse.ProcessResponse(message="List of current processes on {self.name}",
                                                   processes=processes).to_response()
    ...
    def get_process_status(self, process_id):
        ...
        try:
            ...
            return APIResponse.SystemInfoResponse(process.as_dict(attrs=['pid', 'name', 'status'])).to_response()
        except psutil.NoSuchProcess:
            return APIResponse.ErrorResponse('Process not found').to_response(), 404
    ...
    def kill_process(self):
        ...
        if process_id is None:
            return APIResponse.ErrorResponse(f"{__file__} - Missing process_id").to_response()

        try:
            process = psutil.Process(process_id)
            process.terminate()
            return APIResponse.SuccessResponse(f"{__file__} - Process {process_id} terminated").to_response()
        except psutil.NoSuchProcess:
            return APIResponse.ErrorResponse(f"{__file__} - Process not found").to_response()
        except Exception as e:
            return APIResponse.ErrorResponse(f"{__file__} - str(e)").to_response()

    # @app.route('/api/programs/sync', methods=['POST'])
    def sync_programs(self):
        """ Sync programs from JSON configuration """
        if not os.path.exists(CONFIG_PATH):
            return APIResponse.ErrorResponse("Configuration file not found").to_response()

        with open(CONFIG_PATH, 'r') as f:
            try:
                programs = json.load(f)
            except json.JSONDecodeError:
                return APIResponse.ErrorResponse("Invalid JSON format").to_response()

        return APIResponse.ErrorResponse(programs).to_response()
    ...
    def get_programs(self):
        ...
        return APIResponse.SuccessResponse(programs).to_response()
    ...
    def get_system_info(self):
        ...
        return APIResponse.SuccessResponse(system_info).to_response()
    ...
    def get_system_logs(self):
        ...
        try:
            ...
            return APIResponse.ErrorResponse("win32evtlog not available").to_response()
        except Exception as e:
            return APIResponse.ErrorResponse(str(e)).to_response()

        return APIResponse.SuccessResponse("", logs).to_response()
